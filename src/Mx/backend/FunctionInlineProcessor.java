...
public class FunctionInlineProcessor {
    ...
    private final int MAX_LOW_INLINE_INST = 1 << 8;
    private final int MAX_FUNC_INST = 1 << 13;
    ...
    public FunctionInlineProcessor(IRRoot ir) { this.ir = ir; }
    ...
    private IRInstruction inlineFunctionCall(IRFunctionCall funcCallInst){
        IRFunction callerFunc = funcCallInst.getParentBB().getFunc();
        IRFunction calleeFunc = funcBakUpMap.getOrDefault(funcCallInst.getFunc(), funcCallInst.getFunc());
        ...
        if (returnInst.getRetValue() != null)
            newEndBBFirstInst.prependInst(new IRMove(newEndBB, funcCallInst.getDest(), (RegValue) renameMap.get(returnInst.getRetValue())));
                ...
                for (BasicBlock bb : reversePostOrder)
                    for (IRInstruction inst = bb.getFirstInst(), nextInst; inst != null; inst = nextInst){
                        ...
                        if (calleeInfo.numCalled == 0){
                            unCalledFunc.add(((IRFunctionCall) inst).getFunc().getName());
                        }
                    }
                if (thisFuncChanged) {
