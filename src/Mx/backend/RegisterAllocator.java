...
public class RegisterAllocator {
    ...
    private void removeVregNode(VirtualRegister vreg){
        VirtualRegInfo vregInfo = vregInfoMap.get(vreg), neighbourInfo;
    ...
    public void run(){
        for (IRFunction irFunction : ir.getFuncs().values()){
            ...
            while (!vregNodes.isEmpty()){
                ...
                Iterator<VirtualRegister> itertor = vregNodes.iterator();
                VirtualRegister vreg = itertor.next();
                itertor.remove();
                removeVregNode(vreg);
                vregOrder.add(vreg);
            }
                ...
                if (forcedPhysicalRegister != null) {
                    ...
                    if (vregInfo.color == null) {
                        ...
                        if (vregInfo.color == null) {
                            ...
                            if (vregInfo.color == null)
                                vregInfo.color = new StackSlot(irFunction, vreg.getName(), false);
                        }
                    }
                }
            }
