...
public class FunctionScopeScanner extends BaseScopeScanner {
    ...
    public void visit(FuncDeclNode node) {
        ...
        if (currentClassType != null) {
            ...
            if (node.isConstruct() && !(node.getName().equals(((ClassType) currentClassType).getName())))
                throw new SemanticError(node.location(), String.format("Function \"%s\" require return type", node.getName()));
        }

        for (VarDeclNode para : node.getParameterList())
            para.accept(this);
    ...
    public void visit(ClassDeclNode node) {
        ...
        for (FuncDeclNode Node : node.getFuncMember())
            Node.accept(this);
    ...
    public void visit(BlockStmtNode node) {
        ...
        for (Node stmtOrVarDecl : node.getStmtsAndVarDecls())
        {
            ...
            } else if (stmtOrVarDecl instanceof VarDeclNode) stmtOrVarDecl.accept(this);
                   else throw new CompilerError(stmtOrVarDecl.location(), "Invalid node type");
        }
        currentScope = currentScope.getParent();
    }
    ...
    public void visit(WhileStmtNode node) {
        ...
        node.getCond().accept(this);
        if (!(node.getCond().getType() instanceof BoolType))
            throw new SemanticError(node.getCond().location(), "loop condition's type isn't \"bool\"");
    ...
    public void visit(ContinueStmtNode node) {
        if (inLoop <= 0) throw new SemanticError(node.location(), "Continue out of loop statement");
    }

    @Override
    public void visit(BreakStmtNode node) {
        if (inLoop <= 0) throw new SemanticError(node.location(), "Break out of loop statement");
    }
    ...
    public void visit(SuffixExprNode node) {
        ...
        if (!(node.getExpr().getType() instanceof IntType))
            throw new SemanticError(node.location(), String.format("SuffixExpr error"));
    ...
    public void visit(SubscriptExprNode node) {
        ...
        if (!(node.getArr().getType() instanceof ArrayType))
            throw new SemanticError(node.getArr().location(), String.format("arr Error"));
        node.getSub().accept(this);
        if (!(node.getSub().getType() instanceof IntType))
            throw new SemanticError(node.getSub().location(), String.format("sub Error"));
    ...
    public void visit(MemberAccessExprNode node) {
        ...
        else throw new SemanticError(node.location(), String.format("MemberAccess Error"));
    ...
    public void visit(AssignExprNode node) {
        ...
        if (invalidAssignType)
            throw new SemanticError(node.location(), String.format("Assign Type differ"));
